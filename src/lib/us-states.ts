/** US states and DC — value is the postal abbreviation. */
...
/** Normalize Places / free-text state values to a 2-letter code when possible. */
export function normalizeUsState(value: string): string {
  const raw = value.trim();
  if (!raw) return "";
  const upper = raw.toUpperCase();
  if (US_STATES.some((s) => s.value === upper)) return upper;
